...
author: "NomadC"
date: "2025-08-05"
categories: [tflite, tflite-micro, code, tinyml]
image: "image.jpg"
---
...
The TFLite was previously decoupled from TensorFlow and was a standalone library called [LiteRT](https://developers.googleblog.com/en/tensorflow-lite-is-now-litert/) now. 
...
The index range depends on how many inputs you have. The order between the data and the index was reversed as well.
...
I will use the yolov8n example from ultralytics to compare the difference between tflite and tflite-micro.
They provide a [script](https://github.com/ultralytics/ultralytics/blob/main/examples/YOLOv8-TFLite-Python/main.py) to inference yolov8 tflite model and visualize the result, makes it quite convenient.
I make another class that subclass the YOLOv8TFLite example but using tflite-micro python package to do the inference.


If you are interested to follow the example you can refer my repo [tflite_vs_tflite-micro](https://github.com/digital-nomad-cheng/tflite_vs_tflite-micro).
...
![Image 1 Caption](tfl_result.png){.border}
![Image 2 Caption](tflm_result.png){.border}
...
From the example above, we could see that due to the implementation difference TFLite Python API and TFLite Micro Python produced different result. I have seen worse cases than where the tflite result and tflite-micro result diverge a lot this from my past experience.
My suggestion is if you want to check or reproduce your embedded tflite-micro result with python script, **tflite-micro python package is a better choice than tflite python package**. But due to the slow inference speed of tflite-micro package, **it's not suitable for evaluation the whole evaluation dataset.**
